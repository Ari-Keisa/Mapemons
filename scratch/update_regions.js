...
for (const region of regions) {
    const file = path.join(pagesDir, \`\${region}.html\`);
    if (!fs.existsSync(file)) continue;
    
    let content = fs.readFileSync(file, 'utf8');
    
    if (!content.includes('function highlightByAbility')) {
        content = content.replace(/(function highlightByTier\\([\\s\\S]*?highlightTimeout = setTimeout\\(\(\\) => \\{[\\s\\S]*?\\}, 60000\\);\\s*\\})/, '$1\\n' + highlightFunctions);
    }
    
    if (!content.includes('ПОИСК ПО СПОСОБНОСТИ')) {
        content = content.replace(/(highlightByTier\\(tierKey\\);\\s*return;\\s*\\}\\s*\\})/, '$1\\n' + parseLogic);
    }
    
    fs.writeFileSync(file, content);
    console.log(\`Updated \${region}.html\`);
}
